'''
Run ishigami function example for original marom for different tau values.
'''
...
# subprocess.run(["python3", "preproc_short.py"]) # run preproc_short to generate data
...
for j in range(len(tau)):
    ...
    for k in range(rep):
        ...
        for i in range(len(n)):
            # --- original hikrig ---
            err[i,k,j] = hikrig(model,n[i],m[i],k)
etime = time.time()
...
np.savez(f'{npz_dir}/err_naive_tau.npz',    # save error data
